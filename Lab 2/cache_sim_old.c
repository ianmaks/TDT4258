...
uint32_t access_cache(uint32_t** cache, mem_access_t access) {
  for (int i = 0; i < cache_size; i++) {
    for (int j = 0; j < block_size; j++) {
      if (cache[i][j] == access.address) {
...
cache_t* init_cache(uint32_t size, cache_map_t map, cache_org_t org) {
  ...
  cache->blocks = (block_t**) malloc(sizeof(block_t*) * size/block_size);

  uint32_t i = 0;
  for (i = 0; i < size/block_size; i++) {
    cache->blocks[i] = (block_t*) malloc(block_size);
  }
...
void main(int argc, char** argv) {
  ...
  /* Initialize cache */
  cache_t* cache = init_cache(cache_size, cache_mapping, cache_org);
